...
const http = require('http'),
  ...
  proxyDebug = require('debug')('proxy')
    ...
    const proxyReq = (/https/i.test(protocol) ? https : http).request(options, (res) => {
      ...
      const finishCollecting = () => {
        new Promise((fulfill, rejectParsing) => {
          ...
          proxyDebug('serverResData:', serverResData.toString());
          resolve({
            statusCode,
            header: resHeader,
            body: serverResData,
    ...
    */
    ...
    co(fetchReqData)
      ...
      .then(co.wrap(function *() {
        ...
        proxyDebug('finalReqDetail:', finalReqDetail);
        proxyDebug('finalReqDetail.requestData:', finalReqDetail.requestData.toString());
